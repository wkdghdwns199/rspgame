...
<?php	
	...
	if ($member['loginSts'] == 1){echo '<script> alert("로그인이 된 계정입니다."); location.href="../../dashboard/index.html"; </script>';}
	else {
		 $db = mysqli_connect("localhost", "root", "111111", "memberdata");
		 $sql = "UPDATE member SET loginSts=1 WHERE id='{$_POST["userid"]}';";

	  $result = mysqli_query($db, $sql);
	
	//$hash_pw = $member['pw']; //$hash_pw에 POSt로 받아온 아이디열의 비밀번호를 저장합니다. 

	if($password == $member['pw']) //만약 password변수와 hash_pw변수가 같다면 세션값을 저장하고 알림창을 띄운후 main.php파일로 넘어갑니다.
	{
